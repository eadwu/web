(() =>
{
  ...
  let computerStrategies: Array<() => any>;
  ...
  function getValidUserWeapons (user: string): Array<string>
  {
    ...
    if (_window[ `${user}Rocks` ] > 0)
      validWeapons.push("ROCK");
    if (_window[ `${user}Papers` ] > 0)
      validWeapons.push("PAPER");
    if (_window[ `${user}Scissors` ] > 0)
  ...
  function getLeastUserWeapon (user: string): string
  {
    ...
    if (computerPapers <= _window[leastWeapon])
      leastWeapon = `${user}Papers`;
    if (computerScissors <= _window[leastWeapon])
  ...
  function getWeaponWeakness (weaponName: string): string
  {
    ...
    if (/Rock/.test(weaponName) && _window[`${user}Papers`] > 0)
      weaponWeakness = "PAPER";
    else if (/Paper/.test(weaponName) && _window[`${user}Scissors`] > 0)
      weaponWeakness = "SCISSOR";

    return _window[weaponWeakness];
  }
  ...
  */
  ...
  {
    return _window[ weapons[ floor(random() * weapons.length) ] ];
  }
  ...
  function updateScore ()
  {
    ...
    {
      playerScore++;
      _window[ `computer${compSelection}` ] -= 1;
      _window[ `player${compSelection}` ] += 1;
    }
    else if (roundWinner === COMPUTER)
    {
      computerScore++;
      _window[ `player${plrSelection}` ] -= 1;
      _window[ `computer${plrSelection}` ] += 1;
    }
  }
  ...
  _window.game = (choice: string | number) =>
  {
    ...
    if (_window[ reformedChoice ] <= 0 ||
      getValidUserWeapons("player").length <= 0 ||
      getValidUserWeapons("computer").length <= 0)
      return;

    playerSelection = _window[ choice ] || parseInt(choice.toString(), 10);
